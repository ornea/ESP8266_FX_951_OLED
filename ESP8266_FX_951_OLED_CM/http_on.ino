...
//  //Serial.println(F("\n"));
 ...
 //   //Serial.println(F("Closing ..."));
  ...
  if (digitalRead(HEATER_PIN))
    server.sendContent(F("<b>Heater High</b><br>"));
  else
    server.sendContent(F("<b>Heater Low</b><br>"));
  END_PAGE_CUSTOM;
  END_PAGE;

}

void handle_variable_adc(void)
{
  server.sendContent(String(analogRead(A0)));
  //server.sendContent(String(dht.readTemperature()));
}
void handle_variable_mmv(void)
{
  server.sendContent(String(AppConfig.PID[mmv]));//String(analogRead(A0)));
  //server.sendContent(String(dht.readTemperature()));
}
...
//  //Serial.println(F("accessing Build Info"));
  ...
  server.sendContent(F( "<br>IP Address: "));
  server.sendContent(IpAddress2String(WiFi.localIP()));
...
void handle_restart()
{
  ...
  //Serial.println();
  //Serial.println();
  //Serial.println("Someone hit the restart-reboot button. Goodbye!!!");
  doservers();
  delay(1000);
  doservers();
  delay(1000);
