...
#define START_PAGE(hostname,textPageName) server.sendContent( F("<html lang='en'><head><title>PID Control "HOSTNAME"</title>" \
    ...
    "</head><body bgcolor='lightgray'><div align='center'><h1 align='center'>")); server.sendContent(hostname) ; server.sendContent(textPageName); server.sendContent(F("</h1>"));
...
#define END_PAGE server.sendContent(F("<br>Standard Pages: "\
                                      ...
                                      "</div></body></html>\r\n"));
//Leaving out the '/' for href='restart' seems to stop it from entering a boot loop
#define END_PAGE_CUSTOM server.sendContent(F("Custom Pages: "\
    ...
    "<a href='/pid_websockets_charts'>PID websockets charts</a>&nbsp&nbsp"\
